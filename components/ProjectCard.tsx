...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelectProject }) => {
  const { id, title, description, participants, createdAt, originalityScore, category } = project;

  return (
    <div
      onClick={() => onSelectProject(id)}
      className="bg-white rounded-lg shadow-lg overflow-hidden cursor-pointer transform hover:-translate-y-1 transition-all duration-300 relative"
    >
      <div className="p-6">
